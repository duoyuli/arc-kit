...
**例外**：`arc project apply` 在**交互式**且**无 `arc.toml`** 时先走创建向导；**非交互式**且无 `arc.toml` 时，纯文本路径报错并 **exit 1**，`--format json` 时输出 `WriteResult.ok == false` 且 **exit 0**；**已有 `arc.toml`** 时 `apply --format json` 按写入类正常输出。`arc project edit` 不提供一键非交互编辑路径，`--format json` 只返回结构化失败信息，不会打开编辑器。
...
用于：`skill list`（是否 TUI）、`skill install/uninstall`（无名称时是否向导）、`project apply`（无 `arc.toml` 是否创建；有待装 skill 时是否多选目标 agent）、`project edit`（交互式编辑器与 JSON 失败分支）。
...
| `arc mcp info <name>` | `McpInfoOutput`（存在时） |
...
| `project apply` | 有 `arc.toml` 且需装项目 skill 时，**非交互式**须 `--agent` 或 `--all-agents`；**无 `arc.toml` 且首次创建**仅**交互式**向导，`--format json` 只返回 `WriteResult.ok == false`；自动化若要创建仍需手写或拷贝 `arc.toml` |
| `project edit` | 当前仅交互式编辑器；`--format json` 只返回 `WriteResult.ok == false`，不执行编辑；替代方案是编辑 `arc.toml` 后 `arc project apply` |

### 改进优先级（工程）

1. **P0**：为 `project edit` 增加显式参数，或产品层确认长期仅编辑器 + `apply`。
2. **P1**：`project apply` 在无 `arc.toml` 时可选非交互创建参数。
3. **P2**：为 `project apply` 在无 `arc.toml` 时设计显式非交互初始化参数（若产品决定支持）。
