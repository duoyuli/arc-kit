...
用于：`status`（是否弹确认）、`skill list`（是否 TUI）、`skill install/uninstall`（无名称时是否向导）、`project apply`（无 `arc.toml` 是否创建；有待装 skill 时是否多选目标 agent）、`project edit`（仅交互式）。
...
| 成功 | 0 |
| 配置文件解析失败 | 1 |
| 纯文本非交互时 `status` 有缺失 skill | 1 |
| `status --format json` 有缺失 skill | 0（缺失信息见 JSON `missing_skills`） |
| 非交互式缺必要参数 | 1 |
| `arc provider test` 有失败项 | 1 |
| JSON 序列化失败 | 1 |

**与 JSON 的关系**：写入类在「缺 `arc.toml`」等场景可能 **`exit 0` 但 `WriteResult.ok == false`**，自动化须以 JSON 的 `ok` / `message` 为准；`status` 在 **`--format json`** 与**纯文本非交互**路径下退出语义可能不同，集成时须区分。
...
顶层含 `schema_version: "1"`。破坏性变更升版本；兼容新增字段可不升。可选字段避免输出无意义 `null`。消费方应校验 `schema_version`。
...
| 命令 | JSON |
|------|------|
| `arc status` | `StatusOutput`（项目解析失败时仍可输出，`project` 可空） |
