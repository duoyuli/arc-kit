...
- `arc project edit` 只在交互式终端可用；非交互或 `--format json` 只返回失败结果。
...
- Kimi CLI 的 MCP 是全局优先；项目级声明要配合 `--allow-global-fallback` 或 `scope_fallback = "global"`。
...
- “为什么 MCP 没写进去”
  先区分 global / project，再核对目标 agent 是否支持该 transport 和 scope；Kimi 要特别检查 global fallback。
- “为什么 subagent 没生效”
