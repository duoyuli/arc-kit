...
执行 `arc provider use mirror`，Claude Code 和 Codex 的模型供应商同时切换。自动备份，随时可回滚。支持官方直连、镜像代理、中转站、国内平替等。
...
在仓库中放置 `arc.toml`，执行 `arc project apply`，整个团队自动获得 Market & Skill 设置。支持非交互式 `--format json` 输出，CI/CD 友好。
...
当前支持 Claude Code、Codex、Cursor、OpenCode、Gemini CLI、Kimi CLI。安装时自动检测已安装的 agent。

**Q: 安装 Skill 后，各个 Agent 的目录结构会是什么样？**

默认使用软链接安装（OpenClaw 除外，使用目录复制）：

- Claude Code：`~/.claude/skills/<name>` → `~/.arc-cli/skills/<name>`
- Codex：`~/.codex/skills/<name>` → `~/.arc-cli/skills/<name>`
